...
memoryBlock* nextFreeBlock(int size);
  ...
  if(!b_initialized)
    {
      ...
      head->state = 0;
      head->nextBlock = (memoryBlock*) NULL;
      b_initialized = 1;
...
//gibt einen pointer auf den naechsten freien block, der mindestens size gross ist
memoryBlock* nextFreeBlock(int byteCount){
  memoryBlock* current = head;
  while((current->state == 0 || current->dataLength < byteCount) && current != NULL){
...
void* my_malloc(int byteCount)
{
  ...
  memoryBlock* freeBlock = nextFreeBlock(byteCount);
